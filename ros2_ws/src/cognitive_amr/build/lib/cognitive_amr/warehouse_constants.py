...
SHELVES = {
    'A1': {'cx': -3.5, 'cy': -5.5, 'row': 'A'},
    'A2': {'cx': -3.5, 'cy': -3.0, 'row': 'A'},
    'A3': {'cx': -3.5, 'cy': -0.5, 'row': 'A'},
    'A4': {'cx': -3.5, 'cy':  2.0, 'row': 'A'},
    'A5': {'cx': -3.5, 'cy':  4.5, 'row': 'A'},
    'A6': {'cx': -3.5, 'cy':  7.0, 'row': 'A'},
    'B1': {'cx':  3.5, 'cy': -5.5, 'row': 'B'},
    'B2': {'cx':  3.5, 'cy': -3.0, 'row': 'B'},
    'B3': {'cx':  3.5, 'cy': -0.5, 'row': 'B'},
    'B4': {'cx':  3.5, 'cy':  2.0, 'row': 'B'},
    'B5': {'cx':  3.5, 'cy':  4.5, 'row': 'B'},
    'B6': {'cx':  3.5, 'cy':  7.0, 'row': 'B'},
}
...
def get_approach_pose(shelf_id: str, slot: int | None = None) -> tuple:
    ...
    if s['row'] == 'A':
        ...
        ay = s['cy'] - SHELF_HALF_DEPTH - APPROACH_STANDOFF
        yaw = math.pi / 2
    else:
...
# ── Dispatch zones ───────────────────────────────────────────────────────
DISPATCH_ZONE_1 = (8.6, -6.4)    # main outbound
DISPATCH_ZONE_PRIORITY = (8.6, 5.9)   # express / priority orders

# ── Product catalogue ────────────────────────────────────────────────────
# aruco_id must be 0-49 (DICT_4X4_50)
# placed_shelf / placed_slot = PHYSICAL location in SDF
# For the demo, SKU-0042 is physically at A3-slot-3 but DB says A3-slot-2.
PRODUCTS = {
    ...
    # A3: slot-1 has SKU-0012, slot-2 EMPTY (DB says SKU-0042), slot-3 has SKU-0042
    'SKU-0012': {'name': 'Encoder 1000PPR',       'aruco_id': 12, 'placed_shelf': 'A3', 'placed_slot': 1},
    'SKU-0042': {'name': 'Hydraulic Valve Set',   'aruco_id': 42, 'placed_shelf': 'A3', 'placed_slot': 3},
    'SKU-0013': {'name': 'Pressure Sensor',       'aruco_id': 13, 'placed_shelf': 'B3', 'placed_slot': 1},
    'SKU-0014': {'name': 'Motor Driver PCB',      'aruco_id': 14, 'placed_shelf': 'B3', 'placed_slot': 2},
