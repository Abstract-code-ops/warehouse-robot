...
class OperatorInterfaceNode(Node):
    ...
    def _on_status(self, msg: String):
        ...
        with self._lock:
            if '[SCANNING]' in text or 'scanning' in text.lower():
                ...
                pass
            elif '[CONFIRMED]' in text or '[CORRECTING]' in text:
                self._parse_confidence_update(text)
    ...
    def _on_inv_response(self, msg: String):
        ...
        if data.get('ok') and 'shelf_id' in data.get('data', {}):
            d = data['data']
    ...
    def _parse_confidence_update(self, text: str):
        """Attempt to extract product + confidence from status text."""
        for sku in self._slot_state:
            if sku in text:
                if '[CONFIRMED]' in text:
                    self._slot_state[sku]['confidence'] = 'confirmed'
                elif '[CORRECTING]' in text or 'corrected' in text.lower():
                    self._slot_state[sku]['confidence'] = 'confirmed'
